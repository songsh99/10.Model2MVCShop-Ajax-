...
<tr>
    ...
    <td bgcolor="D6D6D6" width="1"></td>
    <td class="ct_write01"> 
        <c:set var="paymentOption" value="${fn:trim(purchase.paymentOption)}" />
        <select name="paymentOption" class="ct_input_g" style="width: 100px; height: 19px">
            <c:choose>
                <c:when test="${paymentOption eq '1'}">
                    <option value="1" ${paymentOption eq '1' ? 'selected' : ''}>현금구매</option>
                </c:when>
                <c:when test="${paymentOption eq '2'}">
                    <option value="2" ${paymentOption eq '2' ? 'selected' : ''}>카드구매</option>
                </c:when>
            </c:choose>
        </select>
    </td>
